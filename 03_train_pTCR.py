...
def train_valid_pTCR(config, config_model):
    ...
    for epoch in range(config["epochs"]):
        ...
        msg = log_format(train_performance, independent_performance=val_performance, epoch=epoch)
        logger.write(msg)
    ...
    externel_set = PTCRDataset(csv_path=config["dataPath"]+"external_set.csv", config=config)
    external_loader = DataLoader(externel_set, batch_size=config["batchSize"], shuffle=False, num_workers=config["numWorker"])
...
if __name__ == "__main__":
    ...
    independent_performance, external_performance = train_valid_pTCR(config, config_model)
